...
class Wave1D_PINN:
    ...
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, data_sampler, kernel_size):
        ...
        # Initialize network weights and biases (classic PINN)
        self.layers = layers
        ...
        # Wave velocity and damping constants
        ...
        self.t_u_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))
        self.x_u_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))

        self.t_ut_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))
        self.x_ut_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))

        self.t_r_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))
        self.x_r_ntk_tf = tf.placeholder(tf.float32, shape=(None, 1))
        ...
        # Total loss
        self.loss = (self.lambda_r_tf * self.loss_res +
                     self.lambda_u_tf * self.loss_bcs +
                     self.lambda_ut_tf * self.loss_ics_u_t +
                     self.lambda_data_tf * self.loss_data)
    ...
    # Classic PINN fully-connected network, input dimension fixed to 2 (t, x)
    ...
    def forward_pass(self, H):
        # Standard fully-connected network: tanh activations, linear output
    ...
    def train(self, nIter=10000, batch_size=200, log_NTK=False, update_weights=False):
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                loss_value, loss_bcs_value, loss_ics_ut_value, loss_res_value, loss_data_value, b_value, c_value = \
                    self.sess.run(
            ...
            if log_NTK:
                ...
                X_ics_batch_ntk, _ = self.fetch_minibatch(self.ics_sampler, self.kernel_size)
                if it % 100 == 0:
                    print("Compute NTK...")
                    tf_dict_ntk = {self.t_u_ntk_tf: X_bc_batch[:, 0:1],
                                   self.x_u_ntk_tf: X_bc_batch[:, 1:2],
                                   self.t_ut_ntk_tf: X_ics_batch_ntk[:, 0:1],
                                   self.x_ut_ntk_tf: X_ics_batch_ntk[:, 1:2],
                                   self.t_r_ntk_tf: X_res_batch[:, 0:1],
                                   self.x_r_ntk_tf: X_res_batch[:, 1:2]}
                    K_u_value, K_ut_value, K_r_value = self.sess.run([self.K_u, self.K_ut, self.K_r], tf_dict_ntk)
                    ...
                    if update_weights:
                        lambda_K_sum = np.trace(K_u_value) + np.trace(K_ut_value) + np.trace(K_r_value)
